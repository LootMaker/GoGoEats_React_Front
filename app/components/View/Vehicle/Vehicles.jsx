...
class Vehicles extends React.Component {
        ...
        if (this.editForm.isValid()) {
            ...
            if (this.state.editOption) {
                ...
                VehicleService.updateVehicle(this.state.selectedVehicle, (res) => {
                    ...
                    var vehicle = _.find(vehicleList.vehicles, (o) => {
                        return o.id == res.id;
                    });

                    _.extend(vehicle, res);
    ...
    _changeField(field, e) {
        ...
        if (field == 'photo') {
            ...
            return;
        } else if (field == 'menu_icon') {
            this._uploadFile(field, this.refs.menu_icon.files[0]);
            return;
        } else if (field == 'map_icon') {
            this._uploadFile(field, this.refs.map_icon.files[0]);
            return;
        }
    ...
    _changeSwitch(field, e) {
        ...
        setTimeout(() => {
            var selectedVehicle = this.state.selectedVehicle;
            if (field == 'switch_status') {
                if ($('#switch_status').prop('checked') == true) {
                    $('#switch_status').prop('checked', false);
                    selectedVehicle.status = false;
                } else if ($('#switch_status').prop('checked') == false) {
                    $('#switch_status').prop('checked', true);
                    selectedVehicle.status = true;
                }
            }
                ...
                <div>
                    <Panel>
                        ...
                        <Table id="table" responsive striped hover className="b0">
                            ...
                            <tbody>
                                {
                                    this.state.vehicleList.vehicles.map((item, i) => {
                                        return (
                                            <tr key={'vehicleTr' + i} className="tr-hover text-center" onClick={this._editVehicle.bind(this, item)}>
                                                ...
                                                <td>{<img src={UtilService.getImageFromPath(item.menu_icon)} className="img-thumbnail thumb48 img-contain img-circle" style={{ backgroundColor: 'white' }} />}</td>
                                                <td>{<img src={UtilService.getImageFromPath(item.map_icon)} className="img-thumbnail thumb48 img-contain img-circle" style={{ backgroundColor: 'white' }} />}</td>
                                                <td><strong>{item.title}</strong></td>
                                                <td>{item.detail}</td>
                                                <td>{item.max_seat}</td>
                                                <td>{item.search_radius} m</td>
                                                <td>{item.status ? <div className="badge p-sm bg-success">Active</div> : <div className="badge p-sm bg-danger">Deactive</div>}</td>
                                                <td className="text-left">{item.description}</td>
                                                <td>{UtilService.getDateTime(item.created_at)}</td>
                                                <td>{UtilService.getDateTime(item.updated_at)}</td>
                                            </tr>
                                        );
                                    })
                                }
                ...
                <aside className="editsidebar">
                    ...
                    <form id="editForm" className="form" data-parsley-validate="">
                        ...
                        <div className="form-group" >
                            ...
                            <Col sm={6}>
                                <input type="number" value={this.state.selectedVehicle.max_seat || ''} placeholder="Max seat" required="required" onChange={this._changeField.bind(this, 'max_seat')} className="form-control" />
                            </Col>
                        </div>
                        <div className="form-group peaks">
                            <label className="control-label col-sm-6 padding" style={{ marginTop: '6px' }}>Search radius(m)</label>
                            <Col sm={6}>
                                <input type="number" value={this.state.selectedVehicle.search_radius || ''} placeholder="Search radius" required="required" onChange={this._changeField.bind(this, 'search_radius')} className="form-control" />
                            </Col>
                        </div>
                        <div className="form-group">
                        ...
                        <div className="form-group">
                            ...
                            <Col md={12} className="padding">
                                <input
                                    ...
                                    data-classinput="form-control inline"
                                    className="form-control filestyle"
                                    onChange={this._changeField.bind(this, 'menu_icon')} />
                            </Col>
                        </div>
                        <div className="form-group">
                            ...
                            <Col md={12} className="padding">
                                <input
                                    ...
                                    data-classinput="form-control inline"
                                    className="form-control filestyle"
                                    onChange={this._changeField.bind(this, 'map_icon')} />
                            </Col>
                        </div>
                    </form>
