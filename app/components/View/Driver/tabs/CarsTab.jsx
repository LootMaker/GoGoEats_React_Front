...
class CarsTab extends React.Component {
    ...
    _updateVehicle(id) {
        ...
        DriverService.updateDriverVehicle(id, vehicle, (res) => {
            var driver = this.state.driver
            driver.driver_vehicles[this.state.vehicleIndex] = vehicle
    ...
    _deleteVehicle(e) {
        ...
        DriverService.deleteDriverVehicle(driver.id, vehicle, (res) => {
            driver.driver_vehicles.splice(this.state.vehicleIndex, 1)
            this.setState({ driver })
    ...
    render() {
                ...
                results.push(
                    <fieldset key={"doc" + i} className="pb mb">
                        <div className="form-group">
                            ...
                            {item.is_required ? '' : <small>{'(optional)'}</small>}
                            ...
                            <Row>
                                <Col sm={12}>
                                    <div className="text-primary">
                                        <cite>{item.is_expired ? 'Expire date: ' + UtilService.getDate(item.expire_date) : ''}</cite>
                                    </div>
                                </Col>
                            </Row>
                        </div>
        ...
        return (
            <div>
                ...
                <Row>
                    {
                        this.props.driver.driver_vehicles.map((item, i) => {
                            return (
                                <Col key={'car' + i} sm={6}>
                ...
                <aside className="editsidebar">
                    ...
                    <form id="editForm" className="form" data-parsley-validate="">
                        <div className="form-group row">
                            ...
                            <Col sm={9}>
                                <select className="form-control" required="required" value={this.state.selectedVehicle.vehicle_id || ''} onChange={this._changeField.bind(this, 'vehicle_id')} >
