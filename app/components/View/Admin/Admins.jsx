...
class Admin extends React.Component {
    ...
    _createAdmin(e) {
        ...
        $('#switch_vefiry').prop('checked', false);
    ...
    _editAdmin(admin, e) {
        this.setState({
            selectedAdmin: _.clone(admin),
            editOption: true,
            formTitle: "Edit Administrator"
        });

        console.log("items data", admin)
        // reset validation
        this.editForm = $('form#editForm').parsley();
        this.editForm.reset();

        $('#switch_vefiry').prop('checked', admin.isVerify);
    ...
    _uploadFile(field, file) {
        ...
        UploadService.uploadFile(data, (res) => {
            ...
            selectedAdmin[field] = res.message;
            // reset state
            this.setState({
                selectedAdmin: selectedAdmin
            });
    ...
    _changeSwitch(field, e) {
        ...
        setTimeout(() => {
            ...
            if (field == 'isVerify') {
                selectedAdmin.isVerify = UtilService.changeSwitchValue($('#switch_vefiry'))
            } else if (field == 'status') {
                selectedAdmin.isVerify = UtilService.changeSwitchValue($('#switch_status'))
            }
                                    ...
                                    this.state.adminList.items.map((item, i) => {
                                        return (
                                            <tr key={'adminTr' + i} className="tr-hover text-center" onClick={this._editAdmin.bind(this, item)}>
                                                ...
                                                <td>{item.isVerify ? <div className="badge p-sm bg-success">Verified</div> : <div className="badge p-sm bg-danger">No</div>}</td>
                ...
                <aside className="editsidebar">
                    ...
                    <form id="editForm" className="form" data-parsley-validate="">
                        ...
                        <div className="form-group">
                            ...
                            <select className="form-control" required="required" value={this.state.selectedAdmin.role_code || ''} onChange={this._changeField.bind(this, 'role_code')} disabled={isCurrentUser(this)} >
                                <option key="" value="">Select role</option>
                                {     
                                    this.state.roles.map((item, i) => {
                                        console.log("currrentUserId ", this.currentUser.id);
                                        console.log("selectedAdminid", this.state.selectedAdmin.id);
                                        console.log("user role code", this.currentUser.role_code);
                                        return (
                        ...
                        <div className="form-group peaks">
                            ...
                            <Col sm={8}>
                                <label className="switch switch-sm">
                                    <input type="checkbox" id="switch_vefiry" onClick={this._changeSwitch.bind(this, 'isVerify')} disabled={isCurrentUser(this)} />
                                    <em></em>
                                </label>
                            </Col>
                        </div>
                    ...
                    <Button bsStyle="success" style={{ position: 'absolute', bottom: '15px', left: '20px', display: (this.currentUser.roleCode == 100 && this.currentUser.id != this.state.selectedAdmin.id) ? 'block' : 'none' }} onClick={this._submitForm.bind(this)} >
                        <i className="fa fa-check fa-lg"></i>
                    </Button>
                    <Button bsStyle="danger" style={{ position: 'absolute', bottom: '15px', left: '90px', display: (this.currentUser.role_code == 100 && this.state.editOption && this.currentUser.id != this.state.selectedAdmin.id) ? 'block' : 'none' }} onClick={this._deleteConfirmAdmin.bind(this)}>
